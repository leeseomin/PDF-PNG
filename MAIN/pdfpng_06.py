...
class PDFImageExtractorApp(tk.Frame):
    ...
    def load_pdf(self):
        ...
        if pdf_file:
            ...
            pdfminer_text_extracted = self.extract_text_with_pdfminer(pdf_file, output_folder)

            if images_extracted and text_extracted and pdfminer_text_extracted:
    ...
    def extract_text_with_pdfminer(self, pdf_path, output_folder):
        try:
            text = extract_text(pdf_path)
            text_file_path = os.path.join(output_folder, "pdfminer_extracted_text.txt")
            with open(text_file_path, "w", encoding="utf-8") as text_file:
                text_file.write(text)
            # Markdown 파일로 저장
            self.save_as_markdown(text_file_path, output_folder)
            return True
        except Exception as e:
            print(f"An error occurred while extracting text with PDFMiner: {e}")
            return False

    def save_as_markdown(self, text_file_path, output_folder):
        markdown_file_path = os.path.join(output_folder, "pdfminer_extracted_text.md")
        with open(text_file_path, 'r', encoding='utf-8') as text_file, \
             open(markdown_file_path, 'w', encoding='utf-8') as markdown_file:
